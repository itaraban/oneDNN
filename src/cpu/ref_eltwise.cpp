    ...
    if (with_drop_out) {
        ...
        parallel_nd(result_parts, [&](dim_t e) {
            ...
            for (int i = e * part_size;
                    i < nstl::min(nelems, (e + 1) * part_size); ++i)
...
status_t ref_eltwise_fwd_t<data_type>::execute_forward_dense(
    ...
    if (with_drop_out) {
        ...
        //const auto p = CTX_IN_MEM(
        //        const float *, DNNL_ARG_ATTR_DROPOUT_PROBABILITY); //pd()->attr()->drop_out_.p;
        const prop_kind_t pk = pd()->get_prop_kind();
        parallel_nd(result_parts, [&](dim_t e) {
            ref_dropout_fwd_t ref_dropout2(
                    *p, pk, omp_get_thread_num() + *seed);
            for (int i = e * part_size;
                    i < nstl::min(nelems, (e + 1) * part_size); ++i)
